...
func TestReElection2A(t *testing.T) {
	...
	l3 := cfg.checkOneLeader()
	log.Printf("Leader is %d\n!", l3)

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	log.Printf("Connect Leader%d\n!", leader2)
	l4 := cfg.checkOneLeader()
	log.Printf("Leader is %d\n!", l4)
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		log.Printf("Disconnect Server%d %d %d\n!", i1, i2, i3)
		...
		cfg.connect(i2)
		cfg.connect(i3)
		log.Printf("Connect Server%d %d %d\n!", i1, i2, i3)
	}
...
func TestRejoin2B(t *testing.T) {
	...
	log.Printf("Entries input: 102, 103, 104\n!")
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		log.Printf("Server %d is victim!", victim)
		if disconnect {
			cfg.disconnect(victim)
			log.Printf("Server %d disconnect!", victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			cfg.crash1(victim)
			log.Printf("Server %d crash!", victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		...
		if disconnect {
			...
			cfg.connect(victim)
			log.Printf("Server %d connect!", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
